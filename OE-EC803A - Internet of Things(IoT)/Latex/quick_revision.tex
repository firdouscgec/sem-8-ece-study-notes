...
\begin{itemize}
  ...
  \item   \textbf{IoT Categories by Scope:} Classifications of IoT deployments based on reach and target user: \textbf{Personal IoT} (B2C processes), \textbf{Group IoT} (shared local workspaces), \textbf{Community IoT} (municipal grids/smart city), and \textbf{Industrial IoT (IIoT)} (mission-critical operations).
\end{itemize}
...
\begin{itemize}
  ...
  \item   \textbf{Weightless Protocol:} An open standard LPWAN communication protocol operating in TV White Space frequencies for long-range, low-power M2M data transfer.
  \item   \textbf{Computing Hierarchy (Mist, Edge, Fog, Cloud):} Distribution of processing based on proximity to data: \textbf{Mist} (on-device MCU), \textbf{Edge} (local LAN gateway), \textbf{Fog} (regional network node), and \textbf{Cloud} (remote server).
\end{itemize}
...
\begin{itemize}
  ...
  \item   \textbf{Instruction Set Architecture (ISA):} The abstract model of a computer defining supported instructions and register structures (e.g., energy-efficient RISC-based \textbf{ARM} vs performance CISC-based \textbf{x86}).
  \item   \textbf{PCIe (Peripheral Component Interconnect Express):} A high-speed serial expansion bus used in IoT to connect modular communication modules (4G/5G, Wi-Fi, LoRa cards).
\end{itemize}
...
\begin{itemize}
  ...
  \item   \textbf{IoT Service Manageability:} The suite of requirements for deploying and maintaining IoT networks at scale, focusing on simple installation, protocol abstraction, and remote state tracking.
\end{itemize}
...
\begin{itemize}
  ...
  \item   \textbf{Reliable Data Transfer Algorithm:} A 4-step WSN/IoT mechanism ensuring telemetry arrives at the sink node reliably via \textbf{Initialization}, \textbf{Message Relaying (hop-by-hop)}, \textbf{Lost Message Detection}, and \textbf{Selective Recovery}.
\end{itemize}
...
\begin{itemize}
  ...
  \item   \textbf{Smart City Architecture Core Elements:} The multi-layer framework of a smart city comprising \textbf{Integrated Information Provider} (data collection), \textbf{Urban Application Platform} (analytics and services), \textbf{Management Center} (command room control), and \textbf{Mobile Unified Service} (citizen mobile/web applications).
\end{itemize}
