\chapter{Embedded Code Development \& Prototype to Reality}
...
\section{Section 4: Reliable Data Transfer Algorithm in IoT/WSN (Q8.4) [5M][]}
...
\begin{itemize}
  \item   \textbf{Action:} Sets up the network routing pathways, resets sequence counters, and establishes synchronization between the sensor nodes and the receiver (sink node).
  \item   \textbf{Role:} Prepares the buffers and routing tables (e.g., using protocols like RPL) so that packets can be labeled and addressed properly.
\end{itemize}

\subsection{2. Message Relaying Phase}
\begin{itemize}
  \item   \textbf{Action:} Telemetry packets are generated by edge sensors and forwarded \textbf{hop-by-hop} through intermediate router nodes to get closer to the destination sink node.
  \item   \textbf{Role:} Ensures that low-power signals can traverse large geographic areas by using multi-hop routing, with each intermediate node validating and relaying the packet.
\end{itemize}

\subsection{3. Lost Message Detection Phase}
\begin{itemize}
  \item   \textbf{Action:} The sink node monitors the incoming streams of data. Because packets contain sequential numbers (e.g., Sequence 1, 2, 4), the sink identifies that a packet is missing when a gap is detected (e.g., packet 3 is missing).
  \item   \textbf{Role:} Minimizes acknowledgment overhead; rather than acknowledging every single packet, the sink monitors gaps.
\end{itemize}

\subsection{4. Selective Recovery Phase}
\begin{itemize}
  \item   \textbf{Action:} The last step in the algorithm. Instead of requesting the sender to retransmit the entire block of data, the sink requests retransmission of \textbf{only the specific missing packets} (e.g., requesting only packet 3).
  \item   \textbf{Role:} Drastically reduces network traffic, channel congestion, and transmitter power consumption on battery-limited sensor nodes.
\end{itemize}
