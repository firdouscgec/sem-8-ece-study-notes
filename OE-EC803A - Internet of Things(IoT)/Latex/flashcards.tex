...
\textbf{Q:} Define and compare the four categories of IoT by scope: Personal, Group, Community, and Industrial IoT.
\tcbline
\textbf{Ans:} \begin{enumerate}
  \item \textbf{Personal IoT:} Consumer-focused B2C processes (e.g., smartwatches, home voice assistants).
  \item \textbf{Group IoT:} Localized shared environments (e.g., connected office lights, smart classroom boards).
  \item \textbf{Community IoT:} Municipal scale benefiting citizens (e.g., smart city air monitoring, bicycle sharing).
  \item \textbf{Industrial IoT (IIoT):} Mission-critical business/factory systems with zero-downtime tolerance (e.g., turbine vibration monitors, smart grid stations).
\end{enumerate}
\end{tcolorbox}
...
\textbf{Q:} What is the Weightless protocol, and what radio spectrum does it utilize?
\tcbline
\textbf{Ans:} An open standard LPWAN protocol optimized for M2M and IoT. It operates in the \textbf{sub-GHz TV White Space (TVWS)} spectrum (unused frequencies between television channels), offering long range (10–30 km), high obstacle penetration, symmetric links, and ultra-low power consumption.
\end{tcolorbox}
...
\textbf{Q:} Differentiate between Mist, Edge, Fog, and Cloud computing in terms of proximity to the end-user.
\tcbline
\textbf{Ans:} \begin{enumerate}
  \item \textbf{Mist Computing:} Runs directly on physical sensor nodes/microcontrollers (closest to data, $<$1 ms latency).
  \item \textbf{Edge Computing:} Runs on local gateways/controllers within the same LAN (1–10 ms latency).
  \item \textbf{Fog Computing:} Runs on regional LAN/MAN nodes or routers (10–100 ms latency).
  \item \textbf{Cloud Computing:} Runs on remote internet data centers (highest processing/storage, 100-500 ms+ latency).
\end{enumerate}
\end{tcolorbox}
...
\textbf{Q:} Compare ARM vs. x86 ISAs, and state the roles of PCIe and Ethernet in IoT hardware.
\tcbline
\textbf{Ans:} \begin{itemize}
  \item \textbf{ARM vs. x86:} ARM is RISC-based, emphasizing energy efficiency for battery-powered edge nodes (e.g. Raspberry Pi). x86 is CISC-based, emphasizing processing throughput at the cost of high power and cooling (e.g. Galileo).
  \item \textbf{PCIe (Peripheral Component Interconnect Express):} A high-speed serial bus used to plug modular wireless cards (Wi-Fi, Bluetooth, 4G/5G, LoRaWAN) into gateways.
  \item \textbf{Ethernet (10/100Mbit):} Provides a physical wired LAN connection that is immune to RF interference, offering highly reliable and low-latency data transmission.
\end{itemize}
\end{tcolorbox}
...
\textbf{Q:} What are the core requirements of IoT Service Manageability, and what role do CSPs play in video/data hosting?
\tcbline
\textbf{Ans:} \begin{itemize}
  \item \textbf{IoT Service Manageability Requirements:}
\end{itemize}
\begin{enumerate}
  \item \textit{Simple/Fast Installation:} Easy out-of-box setup with auto-registration.
  \item \textit{Protocol Abstraction:} Translating diverse local protocols (Modbus, Zigbee) to standard web protocols (MQTT, HTTP).
  \item \textit{State Tracking:} Device twin tracking for offline devices.
\end{enumerate}
\begin{itemize}
  \item \textbf{CSP Roles:} Cloud Service Providers (CSPs) host massive unstructured data and high-definition video streams (e.g. security camera feeds) that local edge devices cannot store or compute.
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[
  ...
  coltitle=black,
  fonttitle=\bfseries\sffamily\small,
  title={Unit VIII $\bullet$ Card 11},
  boxrule=1.2pt,
  arc=2mm,
  outer arc=2mm,
  boxsep=1.5mm,
...
\textbf{Q:} List the four phases of the Reliable Data Transfer Algorithm in IoT/WSN networks.
\tcbline
\textbf{Ans:} \begin{enumerate}
  \item \textbf{Initialization:} Prepares routing tables, synchronizes nodes, and resets sequence counters.
  \item \textbf{Message Relaying:} Sensors forward packets \textbf{hop-by-hop} through router nodes toward the sink.
  \item \textbf{Lost Message Detection:} The receiver (sink) monitors sequence gaps (e.g. 1, 2, 4) to identify missing packets.
  \item \textbf{Selective Recovery:} The sink requests retransmission of \textbf{only the specific missing packets} (e.g. packet 3), minimizing channel congestion and node power consumption.
\end{enumerate}
\end{tcolorbox}
...
\textbf{Q:} List and define the four core architectural elements of a Smart City.
\tcbline
\textbf{Ans:} \begin{enumerate}
  \item \textbf{Integrated Information Provider:} Baseline ingestion layer that aggregates and stores raw sensor data, exposing it via standard APIs.
  \item \textbf{Urban Application Platform:} The engine layer hosting shared software platforms, GIS utilities, and analytics to drive city services (e.g. traffic routing).
  \item \textbf{Management Center:} Command and control room where operators monitor dashboards, configure alert thresholds, and manage operations.
  \item \textbf{Mobile Unified Service:} Touchpoint layer (portals and mobile apps) through which citizens, businesses, and workers interact with smart city services.
\end{enumerate}
\end{tcolorbox}
