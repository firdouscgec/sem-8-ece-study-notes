...
\section{7. Fiber Connectors, Joints, and GRIN-rod Lenses (Q3.8) [5M][]}
...
\subsection{7.1 GRIN-rod Lenses (Graded-Index Rod Lenses)}
A \textbf{GRIN-rod lens} is a cylindrical glass rod with a refractive index profile that decreases continuously and parabolically from the central axis outward.
\begin{itemize}
  \item   \textbf{Working Principle:} Unlike conventional lenses which use curved glass-air interfaces to refract light, a GRIN-rod lens uses internal index grading to bend light rays continuously along sinusoidal paths.
  \item   \textbf{Dimensions:} The majority of GRIN-rod lenses are very compact, with typical diameters in the range of \textbf{0.5 to 2 mm}.
  \item   \textbf{Alignment Position:} The output face of the transmitting fiber is positioned exactly at the \textbf{focal length} (on or near the opposite lens face) of the GRIN-rod lens. This alignment collimates the light, producing a parallel beam with a low divergent angle of between \textbf{1 and 5 degrees}, which minimizes longitudinal misalignment effects.
\end{itemize}

\subsection{7.2 Lens-Coupled Expanded-Beam Connectors}
...
\begin{itemize}
  \item   \textbf{Insertion Loss:} Lens-coupled expanded-beam connectors exhibit average losses of \textbf{0.7 dB} for both single-mode and graded-index fibers.
  \item   \textbf{Divergence Factors:} Several factors cause the collimated beam exiting a GRIN-rod lens to diverge:
\end{itemize}
\begin{enumerate}
  \item \textit{Refractive Index Profile:} Non-ideal parabolic index distributions.
  \item \textit{Chromatic Aberration:} Different wavelengths focus at slightly different spots.
  \item \textit{Size of Fiber Core:} A larger core deviates from a perfect point source at the focal point.
\end{enumerate}
    \textit{   }Note on Lens Cut Length:* The \textbf{lens cut length} determines the collimation state (e.g. quarter-pitch length for collimation) but is a fixed design property; it is not a dynamic factor causing random beam divergence in an aligned lens.
